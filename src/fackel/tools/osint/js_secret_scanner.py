...
def _fetch(url: str, timeout: int) -> str:
    ...
    Each URL (the target and any discovered script URLs) is guarded against
    DNS rebinding before connecting; a private-resolving URL is skipped
    (returns ``""``) rather than crashing the whole scan.
    """
    try:
        guard_request_target(url, "js_secret_scan")
        ...
        return resp.text
    except (requests.RequestException, ToolException):
        return ""
