...
def _add_ffuf_result(result: Any, findings: list[dict[str, Any]]) -> None:
    """Normalise a single ffuf result dict into a finding.

    *result* is untrusted JSON from the ffuf binary, so it is type ``Any``
    and guarded at runtime before use.
    """
