...
from fackel.tools.scanning._wordlists import find_wordlist as _find_wordlist
...
@fackel_tool(args_schema=FeroxbusterInput)
def feroxbuster_scan(  # noqa: C901 - CLI flag assembly + output parsing for one tool
    target: str,
    wordlist: str = "",
    extensions: str = "",
    filter_status: str = "",
    ...
    if filter_status:
        for status_code in filter_status.split(","):
            status_code = status_code.strip()
            if status_code.isdigit():
                cmd.extend(["--filter-status", status_code])
