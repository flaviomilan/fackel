...
def build_graph() -> CompiledStateGraph:  # type: ignore[type-arg]
    ...
    OSINT runs as parallel specialist sub-agents (``Send`` fan-out →
    ``osint_collect``), converging on ``route_after_osint``.

    Vuln scanning runs as a parallel specialist fan-out (``vuln_dispatch`` →
    ``Send`` → ``vuln_specialist`` → ``vuln_collect``) by default, or as the
    sequential specialist path (``vuln_scan``) when per-tool HITL approval is on —
    ``route_after_port_scan`` picks the entry node.
    """
