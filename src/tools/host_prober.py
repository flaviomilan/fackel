...
def probe_host(domain: str) -> str:
    ...
    for port, scheme in ports_to_check.items():
        url = f"{scheme}://{domain}"
        try:
            response = requests.get(url, timeout=5, verify=(scheme == 'https'), allow_redirects=True)
            output.append(f"- {scheme.upper()} (Port {port}): Found - Status Code: {response.status_code}")

            server_header = response.headers.get('Server', 'N/A')
            output.append(f"  - Server: {server_header}")
            found_services = True
        except requests.exceptions.RequestException as e:
            output.append(f"- {scheme.upper()} (Port {port}): Not found or no response.")
